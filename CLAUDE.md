...
- **`pipeline/`** — Candidate sourcing, qualification, and pool management:
  ...
  - `freemium_pool.py` — Freemium candidate selection. `find_freemium_candidate(session, qualifier)` queries `ProfileEmbedding` directly for disqualified+embedded leads not yet dealt in the freemium campaign's department, ranks via `qualifier.rank_profiles()`, and returns the top-1 candidate. Bypasses the Deal-based pool system entirely.
